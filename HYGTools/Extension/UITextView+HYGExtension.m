...
- (CGFloat)fontLineSpace {
    ...
    NSNumber * number = objc_getAssociatedObject(self, &textView_fontLineSpaceKey);
    return number.floatValue;
}

- (BOOL)isFirstLineHeadIndent {

    NSNumber * number = objc_getAssociatedObject(self, &textView_isFirstLineHeadIndentKey);
    return number.boolValue;
}

- (NSString *)hyg_Text {

    return objc_getAssociatedObject(self, &textView_hyg_TextKey);
}
...
- (void)setHyg_text:(NSString *)text {
    if (self.lineSpace ||
        ...
        objc_setAssociatedObject(self, &textView_hyg_TextKey, text, OBJC_ASSOCIATION_COPY_NONATOMIC);
